...
新增：AI 智能筛选（DeepSeek Chat）- 根据自然语言自动生成筛选方案
升级：AI 一键执行 - 支持更丰富的数据处理计划（筛选/排序/去重/TopN/分组聚合/重命名）
"""
...
# 扩展筛选：支持更多操作符
def filter_rows_ext(df, filters):
    """
    根据扩展的过滤器列表筛选行。
    filters: [ { "column": str, "operator": str, "value": any } ]
    支持: >, <, >=, <=, ==, !=, contains, in, between, startswith, endswith, regex, isnull, notnull
    """
    ...
    for f in filters:
        try:
            ...
            if op in {">", "<", ">=", "<=", "==", "!="}:
                # 尝试将两侧都处理为数值进行比较，否则回退原类型比较
                s_num = coerce_numeric(s)
                use_numeric = not s_num.isna().all()
                v = val
                if isinstance(val, str):
                    try:
                        ...
                        pass
                if use_numeric and isinstance(v, (int, float, np.number)):
                    left = s_num
                    right = v
                else:
                    left = s
                ...
                mask = s.astype(str).str.contains(str(val), na=False, regex=True)

            elif op == "in":
                if isinstance(val, str):
                    candidates = [x.strip() for x in val.split(",") if x.strip() != ""]
                elif isinstance(val, (list, tuple, set)):
                    candidates = list(val)
                else:
                    candidates = [val]
                mask = s.isin(candidates)
                ...
                if not s_num.isna().all() and pd.notna(low_num) and pd.notna(high_num):
                    mask = s_num.between(low_num, high_num, inclusive="both")
                else:
                    # 回退为字符串区间比较（字典序）
                    s_str = s.astype(str)
                    mask = (s_str >= str(low)) & (s_str <= str(high))
...
class AIWorker(QThread):
    ...
    def build_prompt(self, meta):
        instruction = (
            "你是一个表格处理助手。根据用户的中文需求，结合提供的列清单与示例值，生成完整的数据处理计划。\n"
            "必须严格输出 JSON，且只输出 JSON，不要包含额外文字或注释。\n"
            "JSON Schema:\n"
            "{\n"
            '  "columns_to_keep": [字符串列名...],\n'
            '  "filters": [ {"column": 列名, "operator": ">"|"<"|">="|"<="|"=="|"!="|"contains"|"in"|"between"|"startswith"|"endswith"|"regex"|"isnull"|"notnull", "value": 任意} ],\n'
            '  "sort": [ {"column": 列名, "order": "asc"|"desc"} ],\n'
            '  "drop_duplicates": {"subset": [列名...], "keep": "first"|"last"|"False"},\n'
            '  "limit": 数字,\n'
            '  "groupby": { "by": [列名...], "aggs": [ {"column": 列名, "agg": "sum"|"mean"|"count"|"min"|"max"} ] },\n'
            '  "rename": { "原列名": "新列名", "...": "..." }\n'
            "}\n"
            "规范说明:\n"
            "- 只使用提供的真实列名；若需求含别名/口语，请映射到最相近的真实列名。\n"
            "- filters.value 约定:\n"
            "  • in: 数组或逗号分隔字符串\n"
            "  • between: [low, high]\n"
            "  • isnull/notnull: 可省略 value\n"
            "- groupby 后的聚合结果列名请使用 \"列名_聚合名\" 扁平化，不要生成多级列索引。\n"
            "- 若筛选值为字符串请保留字符串类型；为数字请使用数字类型。\n"
            "- 仅输出可由上述逻辑实现的指令。\n"
        )
    ...
    def call_llm(self, prompt):
        ...
        payload = {
            "model": self.model,
            "messages": [
                {"role": "system", "content": "You are a helpful assistant specialized in generating JSON-only outputs for data filtering and transformation."},
...
# AI 计划执行工作线程
class AIApplyWorker(QThread):
    status_signal = pyqtSignal(str)
    finished_signal = pyqtSignal(pd.DataFrame)
    error_signal = pyqtSignal(str)

    def __init__(self, df, plan: dict):
        super().__init__()
        self.df = df
        self.plan = plan or {}
    ...
    def run(self):
        try:
            ...
            # 重命名
            rename_map = plan.get("rename") or {}
            if isinstance(rename_map, dict) and len(rename_map) > 0:
                valid_map = {k: v for k, v in rename_map.items() if k in df.columns and isinstance(v, str) and v}
                missing = set(rename_map.keys()) - set(valid_map.keys())
                if missing:
                    emit(f"AI执行: 重命名忽略不存在列: {list(missing)[:5]}{'...' if len(missing)>5 else ''}")
                if valid_map:
                    df = df.rename(columns=valid_map)
                    emit(f"AI执行: 已重命名 {len(valid_map)} 列")

            # 过滤
            filters = plan.get("filters") or []
            if isinstance(filters, list) and len(filters) > 0:
                before = len(df)
                df = filter_rows_ext(df, filters)
                emit(f"AI执行: 过滤行 {before} -> {len(df)}")

            # 去重
            dd = plan.get("drop_duplicates")
            if isinstance(dd, dict) and dd:
                subset = [c for c in (dd.get("subset") or []) if c in df.columns]
                keep = dd.get("keep", "first")
                # 兼容 "False"/False
                if isinstance(keep, str) and keep.lower() == "false":
                    keep_val = False
                elif keep is False:
                    keep_val = False
                elif str(keep).lower() in ("first", "last"):
                    keep_val = str(keep).lower()
                else:
                    keep_val = "first"
                if subset:
                    before = len(df)
                    df = df.drop_duplicates(subset=subset, keep=keep_val)
                    emit(f"AI执行: 去重({subset}, keep={keep_val}) 行 {before} -> {len(df)}")

            # 分组聚合
            gb = plan.get("groupby")
            if isinstance(gb, dict) and gb:
                by = [c for c in (gb.get("by") or []) if c in df.columns]
                aggs = gb.get("aggs") or []
                agg_dict = {}
                for a in aggs:
                    try:
                        col = a.get("column")
                        agg = str(a.get("agg")).lower()
                        if col in df.columns and agg in ("sum", "mean", "count", "min", "max"):
                            agg_dict.setdefault(col, []).append(agg)
                    except Exception:
                        continue
                if by and agg_dict:
                    df = df.groupby(by, dropna=False).agg(agg_dict)
                    # 扁平化列
                    new_cols = []
                    for t in df.columns:
                        if isinstance(t, tuple) and len(t) == 2:
                            new_cols.append(f"{t[0]}_{t[1]}")
                        else:
                            new_cols.append(str(t))
                    df.columns = new_cols
                    df = df.reset_index()
                    emit(f"AI执行: 分组聚合完成，当前形状 {df.shape[0]}x{df.shape[1]}")
            ...
            if isinstance(sort, list) and len(sort) > 0:
                ...
                for s in sort:
                    try:
                        c = s.get("column")
                        o = str(s.get("order", "asc")).lower()
                        if c in df.columns:
                            by.append(c)
                            ascending.append(o != "desc")
                    except Exception:
                        continue
                if by:
                    df = df.sort_values(by=by, ascending=ascending, kind="quicksort")
                    emit(f"AI执行: 排序完成: {list(zip(by, ['asc' if a else 'desc' for a in ascending]))}")

            # 选择列
            cols = plan.get("columns_to_keep") or []
            if isinstance(cols, list) and len(cols) > 0:
                valid = [c for c in cols if c in df.columns]
                if valid:
                    df = df[valid]
                    emit(f"AI执行: 保留列 {len(valid)} 个")
            ...
            self.error_signal.emit(f"AI 执行失败: {str(e)}")
...
class ExcelFilterApp(QMainWindow):
    ...
    def initUI(self):
        ...
        # 硬编码 API 密钥
        default_key = "sk-96f5d6bd84e2470592d84d85e82ffb92"
        self.ai_api_key_edit.setText(default_key)
        self.ai_api_key_edit.setEchoMode(QLineEdit.Password)
        self.ai_prompt_edit = QTextEdit()
        self.ai_prompt_edit.setPlaceholderText("示例：只要上海 2024 年 6 月的销售订单，保留订单号、客户名、金额列，金额>1000，按金额降序取前100行")
        self.ai_parse_button = QPushButton("AI 解析需求")
        self.ai_parse_button.clicked.connect(self.ai_parse)

        # 新增：AI 一键执行
        self.ai_execute_button = QPushButton("AI 一键执行")
        self.ai_execute_button.clicked.connect(self.ai_parse_and_apply)

        ai_buttons_layout = QHBoxLayout()
        ai_buttons_layout.addWidget(self.ai_parse_button)
        ai_buttons_layout.addWidget(self.ai_execute_button)
        ...
        ai_form.addRow(ai_buttons_layout)
    ...
    def show_error(self, message):
        ...
        if hasattr(self, "ai_execute_button"):
            self.ai_execute_button.setEnabled(True)
    ...
    def ai_result_received(self, result):
        ...
        for f in flts:
            try:
                ...
                continue
        if skipped > 0:
            self.update_status(f"AI: 部分条件({skipped})不受旧版UI支持（如 in/between/regex 等），已忽略于UI，但可用‘AI 一键执行’直接应用。")
    ...
    # ========== AI 一键执行 ==========
    def ai_parse_and_apply(self):
        if self.df is None or not self.available_columns:
            QMessageBox.warning(self, "提示", "请先加载数据后再使用 AI 一键执行。")
            return
        base = (self.ai_base_url_edit.text() or "").strip() or "https://api.deepseek.com/v1"
        model = (self.ai_model_edit.text() or "").strip() or "deepseek-chat"
        key = (self.ai_api_key_edit.text() or "").strip()
        ...
        if not prompt:
            QMessageBox.warning(self, "提示", "请输入需求描述。")
            return
        self.ai_execute_button.setEnabled(False)
        self.update_status("AI: 开始解析并执行...")
        self.ai_exec_worker = AIWorker(self.df, self.available_columns, base, model, key, prompt)
        self.ai_exec_worker.status_signal.connect(self.update_status)
        self.ai_exec_worker.finished_signal.connect(self.ai_plan_ready_then_apply)
        self.ai_exec_worker.error_signal.connect(self.show_error)
        self.ai_exec_worker.start()

    def ai_plan_ready_then_apply(self, plan):
        # 打印计划
        try:
            self.log_message("AI 计划(JSON):")
            self.log_message(json.dumps(plan, ensure_ascii=False, indent=2))
        except Exception:
            pass
        # 开始后台执行计划
        self.update_status("AI: 正在后台执行计划...")
        self.ai_apply_worker = AIApplyWorker(self.df, plan)
        self.ai_apply_worker.status_signal.connect(self.update_status)
        self.ai_apply_worker.finished_signal.connect(self.ai_plan_applied)
        self.ai_apply_worker.error_signal.connect(self.show_error)
        self.ai_apply_worker.start()

    def ai_plan_applied(self, result_df):
        self.filtered_df = result_df
        self.update_data_preview()
        self.save_button.setEnabled(True)
        self.ai_execute_button.setEnabled(True)
        self.update_status(f"AI: 执行完成，当前结果 {len(result_df)} 行 x {len(result_df.columns)} 列")
    # ========== AI 一键执行 结束 ==========
